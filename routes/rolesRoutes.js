...
const { roles } = require('../data/dummyData')
...
router.put(`/:id`, idBodyCheck, async (req, res) => {
  try {
    let data = await db.update(req.data.id, req.body, roles)
    res.json(data)
  }
  catch (err) {
...
router.delete(`/:id`, validateRoleId, async (req, res) => {
  try {
    let data = await db.remove(req.data.id, roles)
    if (data <= 0) throw err
    else {
...
async function validateRoleId(req, res, next) {
  try {
    let data = await db.findById(req.params.id, roles)
    req.data = data
    next()
  }
  catch (err) {
